...
let connCfg = {
    filePath: path.join(__dirname, 'sqlite-test-query.db')
};
let conn = new Conn(connCfg);
let sqlMaker = new SqlMaker(-1, -1);

describe('/lib/db-factory/sqlite/query.js', () => {
    after(async () => {
        await new Promise(s => {
            conn.db.close(() => {
                s();
    ...
    describe('.toArray()', () => {
        ...
        it('有表,order,where', async () => {
            await conn.exec([{
                sql: 'INSERT INTO testTable (id,name,password,`key`) VALUES (\'1\',\'name1\',\'password1\',\'key4\')'
            }, {
                sql: 'INSERT INTO testTable (id,name,password,`key`) VALUES (\'2\',\'name2\',\'password2\',\'key3\')'
            }, {
                sql: 'INSERT INTO testTable (id,name,password,`key`) VALUES (\'3\',\'name3\',\'password3\',\'key2\')'
            }, {
                sql: 'INSERT INTO testTable (id,name,password,`key`) VALUES (\'4\',\'name4\',\'password4\',\'key1\')'
            }]);
            let result = await new Query(conn, 'testTable', sqlMaker).order('key').where('id > ?', 2).toArray();
            assert.deepEqual(result, [{
                id: 4,
                name: 'name4',
                password: 'password4',
